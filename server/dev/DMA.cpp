...
{
	this->m_Descriptors[index] = prd;			   // 设置物理内存描述符
	this->m_Descriptors[index].SetEndOfTable(EOT); // 设置EOT位
}
...
/*============================== class DMA ===============================*/
/* static member */
unsigned short DMA::COMMAND_PORT = 0; // 命令寄存器的端口号
unsigned short DMA::STATUS_PORT = 0;  // 状态寄存器的端口号
unsigned short DMA::PRDTR_PORT = 0;	  // PRD Table基地址寄存器的端口号
...
void DMA::Init()
{
	...
	for (i = 0; i < BUS; i++) /* 枚举PCI总线 */
	{
		for (j = 0; j < DEV; j++) /* 枚举每根总线上的物理设备 */
		{
			...
			{
				bool find = 0;
				for (int index = 0; index < 64; index++) // 遍历config数组
				{
					unsigned int ans;
				...
				if (find == 1)
				{
					...
					{
						Utility::Panic("Error: Unsupported Memory Mapped I/O"); // 报错
					}
					return;
				}
			}
...
bool DMA::IsError()
{
	unsigned char value = IOPort::InByte(DMA::STATUS_PORT); // 读取状态寄存器的值
