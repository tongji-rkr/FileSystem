 ...
 * @LastEditTime: 2023-04-02 15:55:21
 */
#ifndef DMA_H
#define DMA_H
...
public:
	void SetBaseAddress(unsigned long phyBaseAddr);	 // 取物理地址的[31 : 1]位写入PRD相应字段,将物理地址字段的Bit(0)置零，起始地址必须双字节对齐
	void SetByteCount(unsigned short bytes);		 // 将DMA传输字节数的第[15 : 1]位写入PRD相应字段,DMA传输字节数必须是偶数
	void SetEndOfTable(bool EOT /* End of Table */); // 设置EOT位
