 ...
 * @LastEditTime: 2023-04-02 15:54:42
 */
#ifndef IOPORT_H
#define IOPORT_H
...
public:
	...
	{
		__asm__ __volatile__("outb %%al, %%dx" ::"d"(port), "a"(data)); // 通过调用汇编指令outb，将data写入到port中
	}

	// 将16比特数据data，写入到指定端口port中
	static inline void OutWord(unsigned short port, unsigned short data)
	{
		__asm__ __volatile__("outw %%ax, %%dx" ::"d"(port), "a"(data)); // 通过调用汇编指令outw，将data写入到port中
	}

	// 将32比特数据data，写入到指定端口port中
	static inline void OutDWord(unsigned short port, unsigned int data)
	{
		__asm__ __volatile__("outl %%eax, %%dx" ::"d"(port), "a"(data)); // 通过调用汇编指令outl，将data写入到port中
	}
